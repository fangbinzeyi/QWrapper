		...
		try{
			...
			for(int i=0;i<flights.length;i++){
				if(null!=flights[i]&&!"".equals(flights[i])){
					...
					info=flightMap.get(id).toString().replace("'", "").replace("newPrice(", "").replace(")", "").split(",");
					...
					//返航
					String [] retflights=retFlightHtml.split("</tbody>");
					for(int k=0;k<retflights.length;k++){
						...
						String[] retinfo=flightMap.get(retid).toString().replace("'", "").replace("newPrice(", "").replace(")", "").split(",");
						...
						flightDetail.setPrice(Double.valueOf(info[1])+Double.valueOf(retinfo[1]));
						...
						baseFlight.setReturnedPrice(Double.valueOf(retinfo[1]));
