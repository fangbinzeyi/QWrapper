...
public class Wrapper_gjdairxn001 implements QunarCrawler{
...
//							    new NameValuePair("ctl00$ContentPlaceHolder1$hfOrigin", "Bandung BDO"),
//							    new NameValuePair("ctl00$ContentPlaceHolder1$HfDestination", "Palembang PLM"),
							    new NameValuePair("rblDirection", "1"),
							    new NameValuePair("ctl00$ContentPlaceHolder1$Curr", "IDR"),
						...
						flag=true;
						String flightNo=this.getFlightNumber(arg1,pairsnumber);
		...
		if(null!=tbody){
			...
			for(int i=0;i<tr.length;i++){
				if(null!=tr[i]&&!"".equals(tr[i])){
					...
					if(td!=null){
						flightNo=StringUtils.substring(td[2], td[2].indexOf(">")+1, td[2].indexOf("</td>"));
					}
				}
			}
		}
...
//		searchParam.setArr("PLM");
		searchParam.setDepDate("2014-07-30");
		searchParam.setTimeOut("60000");
		searchParam.setWrapperid("gjdairhv001");
		searchParam.setToken("");
		Wrapper_gjdairxn001 gjdairxn001=new  Wrapper_gjdairxn001();
		String html = gjdairxn001.getHtml(searchParam);
		ProcessResultInfo result = new ProcessResultInfo();
		result = gjdairxn001.process(html,searchParam);
